...
var factory = function factory(Chip, Input) {
  var Autocomplete = function (_Component) {
    ...
    function Autocomplete() {
      ...
      return _ret = (_temp = (_this2 = _possibleConstructorReturn(this, (_ref = Autocomplete.__proto__ || Object.getPrototypeOf(Autocomplete)).call.apply(_ref, [this].concat(args))), _this2), _this2.state = {
        direction: _this2.props.direction,
        focus: false,
        showAllSuggestions: _this2.props.showSuggestionsWhenValueIsSet,
        query: _this2.query(_this2.props.value)
      }, _this2.handleChange = function (keys, event) {
        var key = _this2.props.multiple ? keys : keys[0];
        var query = _this2.query(key);
        if (_this2.props.onChange) _this2.props.onChange(key, event);
        _this2.setState({ focus: false, query: query, showAllSuggestions: _this2.props.showSuggestionsWhenValueIsSet }, function () {
          _reactDom2.default.findDOMNode(_this2).querySelector('input').blur();
        });
      }, _this2.handleQueryBlur = function () {
        if (_this2.state.focus) _this2.setState({ focus: false });
      }, _this2.handleQueryChange = function (value) {
        _this2.setState({ query: value, showAllSuggestions: false });
      }, _this2.handleQueryFocus = function () {
        if (_this.refs && _this.refs.suggestions) _this2.refs.suggestions.scrollTop = 0;
        _this2.setState({ active: '', focus: true });
      }, _this2.handleQueryKeyDown = function (event) {
        // Clear query when pressing backspace and showing all suggestions.
        var shouldClearQuery = event.which === 8 && _this2.props.showSuggestionsWhenValueIsSet && _this2.state.showAllSuggestions;
        if (shouldClearQuery) {
          _this2.setState({ query: '', active: '' });
          _this2.unselect(_this2.props.value, event);
        }
      }, _this2.handleQueryKeyUp = function (event) {
        if (event.which === 13) {
          var target = _this2.state.active;
          if (!target) {
            target = _this2.props.allowCreate ? _this2.state.query : [].concat(_toConsumableArray(_this2.suggestions().keys()))[0];
            _this2.setState({ active: target });
          }
          _this2.select(event, target);
        }

        if (event.which === 27) _reactDom2.default.findDOMNode(_this2).querySelector('input').blur();

        if ([40, 38].indexOf(event.which) !== -1) {
          var suggestionsKeys = [].concat(_toConsumableArray(_this2.suggestions().keys()));
          var index = suggestionsKeys.indexOf(_this2.state.active) + (event.which === 40 ? +1 : -1);
          if (index < 0) index = suggestionsKeys.length - 1;
          if (index >= suggestionsKeys.length) index = 0;
          _this2.setState({ active: suggestionsKeys[index] });
        }
      }, _this2.handleSuggestionMouseEnter = function (event) {
        _this2.setState({ active: event.target.id });
      }, _this2.handleSuggestionMouseLeave = function (event) {
        _this2.setState({ active: '' });
      }, _this2.select = function (event, target) {
        _events2.default.pauseEvent(event);
        var values = _this2.values(_this2.props.value);
        var newValue = target === void 0 ? event.currentTarget.id : target;
        _this2.handleChange([newValue].concat(_toConsumableArray(values.keys())), event);
      }, _this2.handleClick = function (event) {
        _reactDom2.default.findDOMNode(_this2).querySelector('input').focus();
    ...
    _createClass(Autocomplete, [{
      ...
      value: function renderSuggestions() {
        ...
        var suggestions = [].concat(_toConsumableArray(this.suggestions())).map(function (_ref4) {
          ...
          var className = (0, _classnames8.default)(theme.suggestion, _defineProperty({}, theme.active, _this4.state.active === key));
          if (_this4.state.query && _this4.state.query.trim() == key) {
          ...
          return _react2.default.createElement(
            'li',
            {
              ...
              key: key,
              className: className,
              onMouseDown: _this4.select,
              onMouseEnter: _this4.handleSuggestionMouseEnter,
              onMouseLeave: _this4.handleSuggestionMouseLeave
            },
            _this4.props.template ? _this4.props.template(value) : value
          );
        });
          ...
          suggestions = [_react2.default.createElement(
            'li',
            {
              ...
              onMouseDown: function onMouseDown(event) {
                _this4.select(event, _this4.state.query);
              },
